...
Future<ServerSetupResult?> showServerSetupDialog({
  ...
  final bindKeyController = TextEditingController(text: initialBindKey);
  ...
  try {
    return await showDialog<ServerSetupResult>(
      context: context,
              ...
              actions: [
                Row(
                  children: [
                    Expanded(
                      child: TextButton(
                        onPressed: () => DialogSafety.popIfMounted(ctx),
                      ...
                      child: ElevatedButton(
                        onPressed: isTesting
                            ...
                            : () async {
                                ...
                                if (bindKeyController.text.trim().isEmpty) {
                                  setDialogState(() {
                                    isTesting = false;
                                    testMessage = '请输入绑定 Key';
                                  });
                                  return;
                                }
                                ...
                                if (ok) {
                                  ...
                                  if (!dialogContext.mounted || !ctx.mounted) {
                                    return;
                                  }
                                  DialogSafety.popIfMounted(
                                    ctx,
                                    result: ServerSetupResult(
                                      ip: ip,
                                      port: port,
                                      isWiredMode: isWiredMode,
                                      bindKey: bindKeyController.text.trim(),
                                    ),
                                  );
                                } else {
                                  setDialogState(() {
